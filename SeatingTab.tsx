...
// XLSX is imported dynamically inside the upload handler to avoid bundler resolution issues
...
const SeatingTab = () => {
  ...
  const handleExcelUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      // Load ExcelJS dynamically (avoids build-time import resolution issues in Vite)
      const mod = await import(/* webpackChunkName: "exceljs" */ "exceljs");
      const ExcelJS = (mod && (mod as any).default) ? (mod as any).default : mod;
      const data = await file.arrayBuffer();
      const workbook = new ExcelJS.Workbook();
      await workbook.xlsx.load(Buffer.from(data));
      const sheet = workbook.worksheets[0];

      // Convert worksheet to array of row objects using header row
      const rows: any[] = [];
      if (sheet) {
        ...
        sheet.eachRow((row, rowNumber) => {
          ...
          row.eachCell((cell, colNumber) => {
            const key = (headers[colNumber - 1] || `col_${colNumber}`).toString();
            let val: any = cell.value;
            // ExcelJS may return objects for dates; normalize to simple values
      ...
      const parseExcelDate = (val: any) => {
        ...
        // ExcelJS may provide Date objects
        if (val instanceof Date && !isNaN(val.getTime())) return val.toISOString().slice(0, 10);
        // numbers may be Excel serial dates (fallback)
